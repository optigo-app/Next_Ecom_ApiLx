...
const MaxNavbar = ({ storeinit, logos }) => {
  const {
    islogin,
    setislogin,
    cartCountNum,
  ...
  useEffect(() => {
    ...
    const sortedMenuItems = uniqueMenuItems?.sort(
  ...
  const getMenuApi = async () => {
    ...
    let cacheKey = "";
    if (pricingContext) {
      const eventName = "elvee_maxmenu_menu";
      const { key } = buildMenuCacheKey(
        eventName,
        storeinit,
        pricingContext,
        finalId,
      ...
      try {
        ...
        if (cacheRes?.cached && Array.isArray(cacheRes.data)) {
          console.log("[Elvee MaxMenu Menu] Serving from cache");
