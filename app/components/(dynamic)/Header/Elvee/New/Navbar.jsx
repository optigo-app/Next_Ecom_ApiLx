...
import { AppBar, Toolbar, Box, IconButton, Container, Drawer, Typography, useMediaQuery, useTheme, alpha, Badge, Skeleton } from "@mui/material";
...
const ElveePreNavbar = ({ storeinit, logos }) => {
    const { islogin, setislogin, cartCountNum, setCartCountNum, wishCountNum, setWishCountNum, setCartOpenStateB2C,
        loginUserDetail
    } = useStore();
    ...
    const handelMenu = (param, param1, param2, event, isFilterKey2Ignore) => {
        if (param?.menuname === "Collection" && param?.key === "Auto" && param?.value === "" && Object.keys(param1 || {}).length === 0 && Object.keys(param2 || {}).length === 0) {
            ...
            <motion.div animate={controls} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)} style={{ position: "sticky", top: 0, zIndex: 999 }}>
                <AppBar position="sticky" elevation={0} sx={{ bgcolor: "transparent !important", top: 0 }}>
                    <Container maxWidth={false} disableGutters sx={{ width: "100%", px: 0 }}>
                        <Toolbar sx={{ justifyContent: "space-between", px: { xs: 0, sm: 4 }, minHeight: { xs: 64, sm: 84 }, bgcolor: "transparent !important", color: isHovered || isScrolled ? "#000" : "#fff" }}>
                                                ...
                                                {DynamicMenu?.slice(0, 3).map((item, i) => {
                                                    ...
                                                    return (
                                                        <Box key={item.ProductTypeName} onClick={() => handleTabChange(item.ProductTypeName)} sx={{ position: "relative", px: 1.5, py: 0.6, cursor: "pointer", borderRadius: 999, overflow: "hidden" }}>
                                                            {isActive && <motion.div layoutId="productType-pill" transition={{ type: "spring", stiffness: 420, damping: 30 }} style={{ position: "absolute", inset: 0, borderRadius: 999, background: tabsData[i]?.gradient.bg, color: tabsData[i]?.gradient.color, zIndex: 0 }} />}
                                                            <Typography sx={{ position: "relative", zIndex: 1, fontSize: "0.72rem", fontWeight: isActive ? 700 : 500, letterSpacing: 1, textTransform: "uppercase", color: isHovered || isScrolled ? (isActive ? "#000" : "#6f6f6f") : isActive ? tabsData[i]?.gradient.color : "rgba(255,255,255,0.75)", transition: "color 0.2s ease", userSelect: "none" }}>{item.ProductTypeName}</Typography>
                                                        </Box>
                                                    );
                                                })}
                                            </Box>
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <Box
                                                                                ...
                                                                                sx={{
                                                                                    ...
                                                                                    "&::before": { content: '""', position: "absolute", top: 0, left: 0, right: 0, height: "20px", background: `radial-gradient( ellipse at top, ${activeTabStyle?.gradient?.color}90, transparent )`, filter: "blur(55px)", pointerEvents: "none", borderTopLeftRadius: 8, borderTopRightRadius: 8 },
                                                                                }}
                                                                            >
                                                                                        ...
                                                                                        {item?.param1?.map((section, index) => (
                                                                                            <Box key={index} sx={{ breakInside: "avoid", marginBottom: 2, textAlign: "center" }}>
                                                                                                <Typography
                                                                                                    ...
                                                                                                    onClick={(e) => {
                                                                                                        e.stopPropagation();
                                                                                                        handelMenu({ menuname: item?.menuname, key: item?.param0name, value: item?.param0dataname }, { key: section?.param1name, value: section?.param1dataname }, {}, e, section?.IsFilterKey1Ignore);
                                                                                                        setHoveredItem(null);
                                                                                                    }}
                                                                                                    sx={{ position: "relative", color: section?.menuname === "Collection" ? "#535353" : "#141414", fontWeight: section?.menuname === "Collection" ? 400 : 700, display: "block", letterSpacing: 0.5, textTransform: "capitalize", mb: section?.menuname === "Collection" ? 0 : 1, wordWrap: "break-word", cursor: "pointer", textUnderlineOffset: "0.3rem", "&:hover": { textDecoration: "underline", textUnderlineOffset: "0.3rem" }, fontSize: "0.92rem" }}
                                                                                                >
                                                                                                    {section?.param1dataname}
                                                                                                </Typography>
                                                                                                <Box sx={{ display: "flex", flexDirection: "column", gap: 0.8, alignItems: "center" }}>
                                                                                                    {section?.param2
                                                                                                        ?.filter((p) => p?.param2dataname && p?.param2dataname.trim() !== "")
                                                                                                        .map((param2Item, param2Index) => (
                                                                                                            <Box
                                                                                                                ...
                                                                                                                onClick={(e) => {
                                                                                                                    e.stopPropagation();
                                                                                                                    handelMenu({ menuname: item?.menuname, key: item?.param0name, value: item?.param0dataname }, { key: section?.param1name, value: section?.param1dataname }, { key: param2Item?.param2name, value: param2Item?.param2dataname }, e, param2Item?.IsFilterKey2Ignore);
                                                                                                                    setHoveredItem(null);
                                                                                                                }}
                                                                                                                component={Link}
                                                                                                                sx={{ position: "relative", textAlign: "left", px: 0, color: "#535353", bgcolor: "transparent", border: "none", cursor: "pointer", fontSize: "0.88rem", textDecoration: "none", borderRadius: 1, transition: "all 0.2s ease", "&:hover": { color: "#141414", textDecoration: "underline", textUnderlineOffset: "0.3rem" }, outline: "none" }}
                                                                                                            >
                                                    ...
                                                    {!menuLoading && islogin && (
                                                        <>
                                                            <Box sx={{ position: "relative" }}>
                                                                <Box component={Link} href="/p/NewArrival/?N=TmV3QXJyaXZhbA==" sx={{ px: 2, py: 3, bgcolor: "transparent", border: "none", cursor: "pointer", fontSize: "0.8125rem", fontWeight: 500, letterSpacing: 0.8, textDecoration: "none", transition: "all 0.2s ease", position: "relative", color: isHovered || isScrolled ? "#000" : "#fff", outline: "none", boxShadow: "none" }}>
                                                                    New Arrivals
                                                                </Box>
                                                            </Box>
                                                            <Box sx={{ position: "relative" }}>
                                                                <Box component={Link} href="/offers" sx={{ px: 2, py: 3, bgcolor: "transparent", border: "none", cursor: "pointer", fontSize: "0.8125rem", fontWeight: 500, letterSpacing: 0.8, textDecoration: "none", transition: "all 0.2s ease", position: "relative", color: isHovered || isScrolled ? "#000" : "#fff", outline: "none", boxShadow: "none" }}>
                                                                    Offers
                                                                </Box>
                                                            </Box>
                                                        </>
            ...
            <Drawer anchor="left" open={mobileOpen} onClose={handleDrawerToggle} sx={{ "& .MuiDrawer-paper": { width: { xs: "90%", sm: 380 }, maxWidth: "100%" } }}>
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 2, borderBottom: `1px solid ${alpha("#fff", 0.1)}` }}>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.6 }}>
                        {islogin && (
                            <IconButton onClick={() => setDrawerSearchOpen((prev) => !prev)} sx={{ color: "#000" }}>
                                <SearchIcon style={{ fontSize: "18px", color: "inherit" }} />
                            </IconButton>
                        )}
                        {islogin && (
                            <IconButton sx={{ color: "#000" }} onClick={() => navigateToMenu("/myWishList")}>
                ...
                <MobileMenu activeMenu={activeMenu} menuItems={currentMenuItems.length > 0 ? currentMenuItems : []} handleMobileMenuClick={handleMobileMenuClick} handleMobileBack={handleMobileBack} handelMenu={handelMenu} islogin={islogin} storeinit={storeinit} IsB2BWebsiteChek={IsB2BWebsiteChek} DynamicMenu={DynamicMenu} selectedProductType={selectedProductType} handleTabChange={handleTabChange} />
            </Drawer>
        </>
    );
};
