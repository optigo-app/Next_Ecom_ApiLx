...
const Header = ({ storeinit, logos }) => {
  ...
  // Stable serialized key so pricingContext reference only changes when values actually differ
  const pricingContextRaw = getPricingContext(loginUserDetail, storeinit, islogin);
  const pricingContextKey = JSON.stringify(pricingContextRaw);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const pricingContext = useMemo(() => pricingContextRaw, [pricingContextKey]);
  ...
  const fetchEcomMenu = useCallback(async () => {
    ...
    if (isB2B && !isUserLoggedIn) return;
    if (!pricingContext || isFetchingRef.current) return;
    // Skip if already fetched for the same PackageId to prevent infinite re-fetch
    if (lastFetchedPackageIdRef.current !== null && lastFetchedPackageIdRef.current === pricingContext?.PackageId) return;
    ...
    try {
      ...
      if (storeinit?.IsB2BWebsite === 0) {
        finalID = islogin === false ? visiterID : resolvedDetail?.id || "0";
      } else {
        finalID = resolvedDetail?.id || "0";
      }
      ...
      if (cacheRes?.cached && Array.isArray(cacheRes.data)) {
        ...
        lastFetchedPackageIdRef.current = pricingContext?.PackageId ?? null;
  ...
  useEffect(() => {
    ...
    // NOTE: storeinit intentionally removed from deps — it is captured inside fetchEcomMenu.
    // Adding it here caused double-trigger and infinite loop.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fetchEcomMenu]);
  ...
  const searchDataFucn = (e) => {
    if (e.key === "Enter") {
      if (searchText) {
        ...
        router.push(`/p/${searchText}?S=${encodeObj}`);
        toggleOverlay();
        setSearchText("");
        setSerachShowOverlay(false);
        setDrawerShowOverlay(false);
                  ...
                  {islogin && (
                    <>
                      <Badge badgeContent={cartCountNum} max={1000} overlap={"rectangular"} color="secondary" className="badgeColorFix" style={{ marginInline: "15px" }}>
                        <Tooltip title="Cart">
                          {IsCartNo == 2 ? (
                            <div className="nav_li_smining_Icone" onClick={toggleCartDrawer} style={{ cursor: "pointer" }}>
                              <ShoppingCartOutlinedIcon sx={{ height: "20px", width: "20px" }} />
                            </div>
                          ) : (
                            <Link href="/cartPage" className="nav_li_smining_Icone">
                              <ShoppingCartOutlinedIcon sx={{ height: "20px", width: "20px" }} />
                            </Link>
                          )}
                        </Tooltip>
                ...
                {IsB2BWebsiteChek === 1 ? (
                  islogin === true ? (
                    <>
                      {storeinit?.IsDesignSetInMenu == 1 && (
                        <li>
                          <Link href="/Lookbook" className="nav_li_smining nav_li_smining_Mobile" style={{ cursor: "pointer" }}
                          >
                            {storeinit?.DesignSetInMenu}
                          </Link>
                        </li>
            ...
            <div className="smiling_Top_header_div3">
              <ul className="nav_ul_shop">
                ...
                {IsB2BWebsiteChek == 0 ? (
                  storeinit?.IsPLW ? (
                    ...
                    <>
                      {isMounted && islogin && (
                        <li>
                          <Link href="/account" className="nav_li_smining nav_li_smining_Mobile" style={{ cursor: "pointer" }} >
                            ACCOUNT
                          </Link>
                        </li>
                      )}
                ...
                {isMounted && islogin ? (
                  ...
                  <li>
                    <Link href={"/LoginOption"} className="nav_li_smining" style={{ cursor: "pointer" }} >
                      <span onClick={() => navigate("/LoginOption")}>
                        LOG IN
                      </span>
                    </Link>
                  </li>
                    ...
                    <li>
                      <Badge badgeContent={wishCountNum} max={1000} overlap={"rectangular"} color="secondary" className="badgeColorFix smr_mobileHideIcone">
                        <Tooltip title="WishList">
                          <Link href="/myWishList" className="nav_li_smining_Icone">
              ...
              <div className="smiling_Top_header_div3">
                <ul className="nav_ul_shop">
                  ...
                  {isMounted && islogin ? (
                    ...
                    <li>
                      <Link href={"/LoginOption"} className="nav_li_smining_Fixed" style={{ cursor: "pointer" }} onClick={() => navigate("/LoginOption")}>
                        LOG IN
                      </Link>
                    </li>
                  )}
