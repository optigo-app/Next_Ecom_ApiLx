...
const QuotationJob = () => {
  ...
  const sortedFilteredData = stableSort(
    filterData,
    getComparator(order, orderBy === "PO" ? "lineid" : orderBy),
  );
  ...
  const handleSearch = (
    ...
    data?.forEach((e, i) => {
      ...
      // }
      if (
        e?.Category?.toString()?.toLowerCase() === categories?.toLowerCase() ||
  ...
  const handleRequestSort = (property) => {
    if (property?.toLowerCase() === "sr#") return null;
      ...
      const sortedFilterData = stableSort(
        filterData,
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await getQuotationJobData(
        currencyRate,
        FrontEnd_RegNo,
        customerid,
        data,
      ...
      if (response.Data?.rd) {
        ...
        response?.Data?.rd?.forEach((e, i) => {
          ...
          datass?.push(obj);
          let findStatus = allStatus?.findIndex(
            (ele, ind) => ele?.label === e?.ProgressStatusName,
          );
          let findCategory = allCategory?.findIndex(
            (ele, ind) => ele?.label === e?.Category,
          );
          let findMetalColor = allMetalColor?.findIndex(
            (ele, ind) => ele?.label === e?.MetalColor,
          );
          let findMetalPurity = allMetalPurity?.findIndex(
            (ele, ind) => ele?.label === e?.MetalType,
          );
          if (findStatus === -1) {
            allStatus?.push({
              id: allStatus?.length,
              label: e?.ProgressStatusName,
              value: e?.ProgressStatusName,
            });
          }
          if (findCategory === -1) {
            allCategory?.push({
        ...
        allMetalColor?.unshift({
          id: allMetalColor?.length,
          label: "ALL",
          value: "ALL",
        });
        allMetalPurity?.unshift({
          id: allMetalPurity?.length,
          label: "ALL",
          value: "ALL",
        });
        let allStatus2 = allStatus?.filter(
          (e) => e?.label !== "" && e?.value !== "",
        );
  ...
  return (
    <div className="jobs_Account_smr" onContextMenu={(e) => e.preventDefault()}>
      <Box
        className="smilingSavedAddressMain quotationFiltersText"
        sx={{ padding: "20px" }}
      >
        {!isSmallScreen && (
          <Box
          ...
          >
            ...
            >
              <Button
                variant="contained"
                sx={{ marginBottom: "35px", background: "#7d7f85" }}
                className="muiSmilingRocksBtn QuotationJobAllBtn"
              ...
              <Box sx={{ padding: "0 20px" }}>
                ...
                >
                  <FormControlLabel
                    value="order"
                    className="orderFrom QuotationJobAllBtnSecDate"
                    control={<Radio />}
                    label="Order Date"
                    sx={{ padding: "0 20px 35px 0", marginRight: "0" }}
                  />
                  <FormControlLabel
                    value="prom"
                    className="orderFrom QuotationJobAllBtnSecDate"
                    control={<Radio />}
                    label="Promise Date"
                    sx={{ padding: "0 10px 35px 0", marginRight: "0" }}
                  />
                </RadioGroup>
              </Box>
              <Box
              ...
              <Box
                ...
                className="searchbox QuotationJobAllBtnSec"
              >
                <TextField
                  id="standard-basic"
                  label="Search"
              ...
              >
                ...
                >
                  <PrintIcon sx={{ color: "#fff !important" }} />
                </Button>
              </Box>
              ...
              >
                ...
                >
                  ...
                  >
                    <ListIcon
                      sx={{
                        color: view === "list" ? "#fff !important" : "inherit",
                      }}
                    />
                  </ToggleButton>
                  <ToggleButton
            ...
            >
              <Box
                sx={{
                  border: view === "list" && "1px solid #d6d6d6",
                  height: "117px",
                }}
              >
                {hoverImg !== "" && (
                    ...
                    >
                      <ToggleButton value="list">
                        <ListIcon
                          sx={{
                            color:
                              view === "list" ? "#fff !important" : "inherit",
                          }}
                        />
                      </ToggleButton>
                      <ToggleButton value="grid">
                        <CollectionsIcon
                          sx={{
                            color:
                              view === "grid" ? "#fff !important" : "inherit",
                          }}
                        />
                      </ToggleButton>
                    </ToggleButtonGroup>
                    ...
                    {view === "list" ? (
                      ...
                      />
                                ...
                                {columns?.slice(1)?.map((column) => {
                                  const ExcludeThisField = [
                                    "Total Qty",
                                    "Supplied",
                                  ];
                                  const isExcluded = ExcludeThisField?.includes(
                                    column?.label,
                                  );
                                  if (isExcluded) {
                                    return null;
                                    ...
                                    >
                                      ...
                                      {orderBy === column.id ? (
                                        <span
                                          ...
                                          className="sorticon_ma_span_SMR"
                                        >
                                          {orderBy === column?.id && (
                                            <CustomSortIcon order={order} />
                                          )}
                                        </span>
                                      ) : null}
                                    </TableCell>
                                  ...
                                  ?.map((row, rowIndex) => {
                                    ...
                                    return (
                                      <TableRow
                                        ...
                                        sx={{
                                          bgcolor: row.isJobSelected
                                            ? "rgba(0, 0, 0, 0.1)"
                                            : "transparent", // Subtle gray on selection
                                        }}
                                        onMouseOver={(e) =>
                                          setHoverImg(row?.imgsrc)
                                        }
                                  ...
                                  <Box sx={{ padding: "10px 5px" }}>
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        {formatter(products?.FinalAmount)}
                                      </Typography>
                                    </Box>
                                    <Box
                                      sx={{
