...
const DesignWiseSalesReport = () => {
  ...
  const [netWtSlider, setNetWtSlider] = useState([
    netWtLimit?.min,
    netWtLimit?.max,
  ]);
  const [grossWtSlider, setGrossWtSlider] = useState([
    grossWtLimit?.min,
    grossWtLimit?.max,
  ]);
  ...
  const [purchaseCount, setPurchaseCount] = useState(
    purchaseCountList?.length > 0 ? purchaseCountList[0]?.value : null,
  );
  const [metalList, setMetalList] = useState([]);
  const [metal, setMetal] = useState(
    metalList?.length > 0 ? metalList[0]?.value : null,
  );

  const [productTypeList, setProductTypeList] = useState([]);
  const [productType, setProductType] = useState(
    productTypeList?.length > 0 ? productTypeList[0]?.value : null,
  );

  const [metalColorList, setMetalColorList] = useState([]);
  const [metalColor, setMetalColor] = useState(
    metalColorList?.length > 0 ? metalColorList[0]?.value : null,
  );

  const [categorylist, setCategorylist] = useState([]);
  const [category, setCategory] = useState(
    categorylist?.length > 0 ? categorylist[0]?.value : null,
  );

  const [subCategoryList, setSubCategoryList] = useState([]);
  const [subCategory, setSubCategory] = useState(
    subCategoryList?.length > 0 ? subCategoryList[0]?.value : null,
  );
    ...
    if (newValue[1] - newValue[0] < minDistanceNetWt) {
      if (activeThumb === 0) {
        const clamped = Math.min(
          newValue[0],
          netWtLimit?.max - minDistanceNetWt,
        );
        handleSearch(
          event,
          fromDate,
  ...
  const handleGrossWtSlider = (event, newValue, activeThumb) => {
    ...
    if (newValue[1] - newValue[0] < minDistanceGrossWt) {
      if (activeThumb === 0) {
        const clamped = Math.min(
          newValue[0],
          netWtLimit?.max - minDistanceGrossWt,
        );
  ...
  const handleSearch = (
      ...
      if (!fromdates?.includes(undefined) && !todates?.includes(undefined)) {
        let salescount = dataRd2?.reduce((acc, cObj) => {
          ...
          if (moment(fromdat).isSameOrBefore(todat)) {
            ...
            if (
              e?.designno === cObj?.designno &&
              (isBetween || cutDat.isSame(fromdat) || cutDat.isSame(todat))
            ) {
        ...
        let salescount = dataRd2?.reduce(
          (acc, cObj) =>
            e?.designno === cObj?.designno ? acc + cObj?.salescount : acc,
          0,
        );
        if (salescount !== 0) {
          let obj = { ...e };
    ...
    datass?.forEach((e, i) => {
      ...
      if (
        netWt1 === netWtLimit?.min ||
        netWt2 === netWtLimit?.max ||
        netWt1 < netWtLimit?.min ||
        netWt2 > netWtLimit?.max
      ) {
      ...
      if (
        grossWt1 === grossWtLimit?.min ||
        grossWt2 === grossWtLimit?.max ||
        grossWt1 < netWtLimit?.min ||
        grossWt2 > netWtLimit?.max
      ) {
      ...
      // }
      if (
        e?.MetalType?.startsWith(Metal_Name) ||
        Metal_Name?.toLowerCase() === "all"
      ) {
        flags.metal = true;
      }
      if (
        e?.ProductType?.startsWith(product_Type) ||
        product_Type?.toLowerCase() === "all"
      ) {
        flags.productType = true;
      }
      if (
        e?.MetalColor?.startsWith(metal_Color) ||
        metal_Color?.toLowerCase() === "all"
      ) {
        flags.metalColor = true;
      }
      if (
        e?.Categoryname?.startsWith(category_name) ||
        category_name?.toLowerCase() === "all"
      ) {
        flags.category = true;
      }
      if (
        e?.SubCategoryname?.startsWith(sub_category) ||
        sub_category?.toLowerCase() === "all"
      ) {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await getDesignWiseSalesReport(
        currencyRate,
        FrontEnd_RegNo,
        customerid,
        data,
      ...
      if (response?.Data?.rd) {
        ...
        response?.Data?.rd?.forEach((e, i) => {
          ...
          if (minNetWt === 0 || minNetWt > e?.DesignNetWt) {
            minNetWt = e?.DesignNetWt + 0.0;
          }
          if (maxNetWt === 0 || maxNetWt < e?.DesignNetWt) {
            maxNetWt = e?.DesignNetWt + 0.0;
          }
          ...
          let findPurCouArr = purchaseCountArr?.findIndex(
            (ele, index) => ele?.label === e?.salescount,
          );
          if (findPurCouArr === -1) {
            if (String(e?.salescount) !== "") {
              purchaseCountArr.push({
                id: purchaseCountArr?.length,
                label: e?.salescount,
                value: e?.salescount,
              });
            }
          }
          let findMetal = metalLists?.findIndex(
            (ele, ind) => ele?.label === e?.MetalType,
          );
          if (findMetal === -1) {
            if (e?.MetalType?.trim() !== "") {
              metalLists.push({
                id: metalLists?.length,
                label: e?.MetalType,
                value: e?.MetalType,
              });
            }
          ...
          let findProductType = productTypeLists?.findIndex(
            (ele, ind) => ele?.label === e?.ProductType,
          );
          if (findProductType === -1) {
            if (e?.ProductType?.trim() !== "") {
              productTypeLists.push({
                id: productTypeLists?.length,
                label: e?.ProductType,
                value: e?.ProductType,
              });
            }
          ...
          let findMetalColor = metalColorLists?.findIndex(
            (ele, ind) => ele?.label === e?.MetalColor,
          );
          if (findMetalColor === -1) {
            if (e?.MetalColor?.trim() !== "") {
              metalColorLists.push({
                id: metalColorLists?.length,
                label: e?.MetalColor,
                value: e?.MetalColor,
              });
            }
          ...
          let findCategory = categoryLists?.findIndex(
            (ele, ind) => ele?.label === e?.Categoryname,
          );
          if (findCategory === -1) {
            if (e?.Categoryname?.trim() !== "") {
              categoryLists.push({
                id: categoryLists?.length,
          ...
          let findSubCategory = subCategoryLists?.findIndex(
            (ele, ind) => ele?.label === e?.SubCategoryname,
          );
          if (findSubCategory === -1) {
            if (e?.SubCategoryname?.trim() !== "") {
              subCategoryLists.push({
                id: subCategoryLists?.length,
        ...
        productTypeLists?.unshift({
          id: productTypeLists?.length,
          label: "ALL",
          value: "ALL",
        });
        productTypeLists?.length > 0 &&
          setProductType(productTypeLists[0]?.value);
        metalColorLists?.unshift({
          id: metalColorLists?.length,
          label: "ALL",
          value: "ALL",
        });
        ...
        subCategoryLists?.length > 0 &&
          setSubCategory(subCategoryLists[0]?.value);
      ...
      <Box className="designWiseSalesReport">
        {!isSmallScreen && (
          <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center" }}>
            ...
            <Box
              sx={{ paddingRight: "25px", paddingBottom: "10px" }}
              className="searchDesignWiseSalesReport"
            >
              <Button
                variant="contained"
                className="muiSmilingRocksBtn"
            ...
            <Box
              sx={{ marginRight: "45px", paddingBottom: "10px", width: 190 }}
            >
            ...
            <Box sx={{ paddingRight: "15px", paddingBottom: "0px" }}>
              <Button
                ...
                sx={{ background: "#7d7f85" }}
                className="muiSmilingRocksBtn"
                onClick={() => handleDWSRprint(filterData)}
              >
                <PrintIcon titleAccess="click here for print" />
              </Button>
            </Box>
          </Box>
        )}
              ...
              <AccordionDetails style={{ padding: "15px 10px" }}>
                ...
                >
                  <Button
                    variant="contained"
                    size="small"
                    sx={{ background: "#7d7f85", padding: "6px 20px" }}
                    className="muiSmilingRocksBtn"
                    onClick={(eve) => resetAllFilters(eve)}
                  >
                    All
                ...
                >
                  ...
                  >
                    ...
                    >
                      <SearchIcon
                        sx={{ color: "#fff !important", marginRight: "5px" }}
                      />{" "}
                      Search
                    </Button>
                  </Box>
          ...
          <>
            {filterData?.length > 0 ? (
              <Box
                ...
                className="designWiseSalesProducts"
              >
                  ...
                  >
                      ...
                      <Box sx={{ minheight: "271px" }}>
                        {isLoading === false ? (
                          ...
                          <Skeleton
                            variant="rectangular"
                            width={"100%"}
                            style={{ marginBottom: "76px", height: "310px" }}
                          />
                        )}
                      </Box>
                    </Box>
                    <Box>
                      ...
                      >
                        ...
                        >
                          Dia Pcs/Wt:{" "}
                          {NumberWithCommas(products?.diamondpcs, 0)} /{" "}
                          {NumberWithCommas(products?.diamondwt, 3)}
                        </Typography>
                        <Typography
                          style={{ fontSize: "12px", textAlign: "start" }}
                        >
                          Cs Pcs/Wt:{" "}
                          {NumberWithCommas(products?.colorstonepcs, 0)} /{" "}
                          {NumberWithCommas(products?.colorstonewt, 0)}
                        </Typography>
                      </Box>
                      <Box
                        sx={{
                      ...
                      >
                        ...
                        >
                          Purchase Count:{" "}
                          {NumberWithCommas(products?.salescount, 0)}
                        </Typography>
                      </Box>
                    </Box>
                  </div>
