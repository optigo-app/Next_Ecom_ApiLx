...
import { checkMonth, formatAmount } from "@/app/(core)/utils/Glob_Functions/AccountPages/AccountPage";
    ...
    <div className="ledger_Account_RPJ">
      <div>
        <div className="border_Acc">
          {moment(showStartDate).format("DD MMM YYYY") !== "Invalid date" && (
            <div className="p_2_acc ps_4_acc border_bottom_acc fs_Al_mq" style={{ letterSpacing: "1px" }}>
              Account Detail for &nbsp; <b className="fs_Al_mq">{userName}</b>
              &nbsp; Period of &nbsp;<b className="fs_Al_mq">{moment(showStartDate).format("DD MMM YYYY") === "Invalid date" ? "" : moment(showStartDate).format("DD MMM YYYY")}</b>&nbsp; to &nbsp;<b className="fs_Al_mq">{moment(showEndDate).format("DD MMM YYYY") === "Invalid date" ? "" : moment(showEndDate).format("DD MMM YYYY")}</b>&nbsp;
            </div>
          )}
              ...
              <Stack direction="column" spacing={2} sx={{ width: "100%", p: 1 }}>
                ...
                <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ xs: "stretch", md: "center" }} flexWrap="wrap">
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: { xs: "100%", md: "auto" } }}>
                    <Typography sx={{ fontSize: "14px", fontFamily: "Spectral-Regular", minWidth: 50 }}>Date :</Typography>
                  ...
                  {/* To Date */}
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: { xs: "100%", md: "auto" } }}>
                    <Typography sx={{ fontSize: "14px", fontFamily: "Spectral-Regular", minWidth: 50 }}>To :</Typography>
                  ...
                  {[30, 60, 90]?.map((days) => (
                    <Button
                      ...
                      sx={{
                        minWidth: 50,
                        height: 36,
                        borderRadius: 1, // slight rounding (not full circle)
                        border: `1px solid ${selectedDays === days ? "#7d7f85" : "#e8e8e8"}`,
                        background: selectedDays === days ? "#7d7f85" : "#fff",
                        color: selectedDays === days ? "#fff" : "#7d7f85",
                      }}
                ...
                {filterArray?.length > 0 && (
                  ...
                  >
                    <Box
                      onClick={() => downloadExcelLedgerData()}
                      sx={{
                        display: "flex",
                        alignItems: "center",
                        gap: 1,
          ...
          >
            ...
            >
              ...
              <Box sx={{ display: "flex", alignItems: "center", width: "100%", justifyContent: "space-between" }}>
                <Typography sx={{ fontFamily: "Spectral-Regular", fontSize: "0.9rem" }}>Balance Gold :</Typography>
              ...
              {/* Balance Diamond */}
              <Box sx={{ display: "flex", alignItems: "center", width: "100%", justifyContent: "space-between" }}>
                <Typography sx={{ fontFamily: "Spectral-Regular", fontSize: "0.9rem" }}>Balance Diam. :</Typography>
              ...
              {/* Balance Amount */}
              <Box sx={{ display: "flex", alignItems: "center", width: "100%", justifyContent: "space-between" }}>
                <Typography sx={{ fontFamily: "Spectral-Regular", fontSize: "0.9rem" }}>Balance Amount :</Typography>
          ...
          {loaderAC ? (
            <Box sx={{ display: "flex", justifyContent: "center", paddingTop: "10px", paddingBottom: "30px" }}>
              <CircularProgress className="loadingBarManage" />
            </Box>
          ) : (
            <div style={{ margin: "8px", overflow: "auto" }}>
              <div>
                <AccountLedgerExcel filterArray={filterArray} credit_curr_diff={credit_curr_diff} credit_amt_diff={credit_amt_diff} credit_mg_diff={credit_mg_diff} credit_dia_diff={credit_dia_diff} debit_curr_diff={debit_curr_diff} debit_amt_diff={debit_amt_diff} debit_mg_diff={debit_mg_diff} debit_dia_diff={debit_dia_diff} resultTotal={resultTotal} currencySymbol={currencySymbol} currencyRate={currencyRate} />
              </div>
              <table className="w_100_acc">
                <thead className="w_100_acc border_Acc">
                  <tr className="w_100_acc border_bottom_acc fs_td" style={{ width: "100%", fontFamily: "Spectral-Regular" }}>
                    <td className="fw_bold_acc text_center_acc border_end_acc" colSpan={7}>
                      DEBIT
                    </td>
                    <td className="fw_bold_acc text_center_acc" colSpan={11}>
                      CREDIT
                    </td>
                  </tr>
                ...
                <tbody className="fs_td">
                  {Math.abs(debit_amt_diff) === 0 && Math.abs(debit_curr_diff) === 0 && Math.abs(debit_dia_diff) === 0 && Math.abs(debit_mg_diff) === 0 && Math.abs(credit_amt_diff) === 0 && Math.abs(credit_curr_diff) === 0 && Math.abs(credit_mg_diff) === 0 && Math.abs(credit_dia_diff) === 0 ? (
                    ""
                  ) : (
                    <tr className="border_Acc fw_bold_acc" style={{ fontFamily: "Spectral-Regular" }}>
                      ...
                      <td className="border_end_acc p_1_acc  ps_1_acc" align="center">
                        Opening
                      </td>
                      ...
                      <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ minWidth: "100px" }}>
                        {Math.abs(debit_curr_diff) === 0.0 ? (
                          ""
                        ) : (
                          <>
                            <span dangerouslySetInnerHTML={{ __html: currencyCode }}></span>&nbsp;{formatAmount(Math.abs(debit_curr_diff))}
                          </>
                        )}
                      </td>
                      <td className="border_end_acc p_1_acc text_center_acc"></td>
                      <td className="border_end_acc p_1_acc text_center_acc"></td>
                      <td className="border_end_acc p_1_acc text_start_acc ps_1_acc" align="center">
                        Opening
                      </td>
                      ...
                      <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ minWidth: "100px" }}>
                        {Math.abs(credit_curr_diff) === 0.0 ? (
                          ""
                        ) : (
                          <>
                            <span dangerouslySetInnerHTML={{ __html: currencyCode }}></span>&nbsp;{formatAmount(Math.abs(credit_curr_diff))}
                          </>
                        )}
                      </td>
                        ...
                        <>
                          {e === "Data Not Present" ? (
                            ...
                            <tr className="border_Acc" key={e?.id} style={{ fontFamily: "Spectral-Regular" }}>
                              <td className="border_end_acc p_1_acc text_center_acc" style={{ verticalAlign: "top" }}>
                                {e?.IsDebit === 0 ? "" : e?.EntryDate}
                              </td>
                              <td className="border_end_acc p_1_acc text_start_acc ps_1_acc" style={{ verticalAlign: "top" }}>
                                {e?.IsDebit === 0 ? "" : e?.particular}
                              </td>
                              <td className={`border_end_acc p_1_acc text_start_acc ps_1_acc ${e?.PrintUrl === "" ? "" : "text-primary text-decoration-underline"}  `} style={{ cursor: "pointer", verticalAlign: "top" }} onClick={() => (e?.PrintUrl === "" ? "" : e?.IsDebit === 1 && window.open(atob(e?.PrintUrl)))}>
                              ...
                              <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ verticalAlign: "top" }}>
                                {e?.IsDebit === 0 ? "" : e?.metalctw === 0 ? "" : e?.metalctw}
                              </td>
                              <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ verticalAlign: "top" }}>
                                {e?.IsDebit === 0 ? "" : e?.diamondctw === 0 ? "" : e?.diamondctw}
                              </td>
                              <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ minWidth: "100px", verticalAlign: "top" }}>
                                {" "}
                                {e?.IsDebit !== 0 && <span dangerouslySetInnerHTML={{ __html: currencyCode }}></span>} {e?.IsDebit === 0 ? "" : ` ${formatAmount(e?.Currency) === "NaN" ? "" : formatAmount(e?.Currency)} `}
                              </td>
                              ...
                              <td className={`border_end_acc p_1_acc text_start_acc ps_1_acc ${e?.PrintUrl === "" ? "" : "text-primary text-decoration-underline"}`} onClick={() => (e?.PrintUrl === "" ? "" : e?.IsDebit === 0 && window.open(atob(e?.PrintUrl)))} style={{ cursor: "pointer", verticalAlign: "top" }}>
                                {e?.IsDebit === 0 ? (e?.referenceno === "" ? e?.voucherno : e?.referenceno) : ""}
                              </td>
                              {/* <td className='border_end_acc p_1_acc text_start_acc ps_1_acc ' style={{ cursor: 'pointer' }}>{e?.IsDebit === 0 ? e?.referenceno === '' ? e?.voucherno : e?.referenceno : ''}</td> */}
                              ...
                              <td className="border_end_acc p_1_acc text_end_acc pe_1_acc" style={{ minWidth: "100px" }}>
                                {" "}
                                {e?.IsDebit === 0 && <span dangerouslySetInnerHTML={{ __html: currencyCode }}></span>} {e?.IsDebit === 0 ? ` ${e?.Currency === 0 ? " 0.00" : formatAmount(e?.Currency)}` : ""}
                              </td>
...
export default AccountLedgerX2;
...
//                         <div className='custom_flex_class' style={{ gap: '20px', alignItems: 'center' }}>
...
//                     <div className='custom_flex_class' style={{ gap: '20px' }}>
//                         <div className=' px_2_al d_flex_Acc flex_column align_items_center mb_2_acc p_1_acc w_100_acc'>
...
//                         </div>
//                         <div className=' px_2_al d_flex_Acc flex_column align_items_center mb_2_acc w_100_acc'>
...
//                     {
//                         <div className='fs_al2' style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flexWrap: 'wrap', marginBottom: '0px', width: '100%', padding: '16px' }}>
//                             <div className='mb_acc'>
//                                 <Box sx={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: '16px' }} className="">
//                                     <Box sx={{ display: "flex", alignItems: "center", paddingRight: "15px" }} className="QuotePadSec date_margin_acc">
...
//                                                     name="date"
//                                                     id="enddate"
//                                                     slotProps={{ textField: { size: 'small', sx: { '& .MuiInputBase-root': { borderRadius: '0' } } } }}
//                                                 />
//                                             </LocalizationProvider>
//                                         </Box>
//                                     </Box>
...
//                                 <Box sx={{ paddingRight: "15px", display: 'flex', alignItems: 'center' }}>

//                                     <Button variant='contained' className='muiSmilingRocksBtn' title='search here'
//                                         sx={{ padding: "7.5px 10px", minWidth: "max-content", background: "#7d7f85", borderRadius: 0 }}
//                                         onClick={(e) => handleSearchBtn(e, fromDate, toDate, selectedDays)}
//                                     >
...
//                             </div>
//                             <Box sx={{ paddingRight: "15px", display: 'flex', alignItems: 'center' }}>
//                                 <Button variant="contained" className="muiSmilingRocksBtn" sx={{ background: "#7d7f85", fontFamily: "Spectral-Regular", display: "flex", alignItems: "center", marginBottom: 0, padding: "6.5px 24px", borderRadius: 0, '&:hover': { background: '#7d7f85' } }}
//                                     onClick={() => backToInitial3()}>
//                                     ALL
//                                 </Button>
//                             </Box>
//                             <Box sx={{ paddingRight: "15px", display: 'flex', alignItems: 'center' }} className="pad_r_acc center_acc w_all_acc">
...
//                                             paddingTop: '0px', paddingBottom: '0px', marginBottom: '8px', borderRadius: 0, background: '#fff', color: '#7d7f85'
//                                         }}
...
//                                             title={`${days} days`}
//                                             style={{ border: `1px solid ${selectedDays === days ? '#7d7f85' : '#e8e8e8'}`, borderRadius: 0, background: selectedDays === days ? '#7d7f85' : '#fff', color: selectedDays === days ? '#fff' : '#7d7f85' }}
...
//                                         onClick={() => handleNextDays()}
//                                         style={{ borderRadius: 0, background: '#fff', color: '#7d7f85' }}
//                                     >&gt;</button>
//                                 </div>
//                             </Box>
...
//                             </div>
//                             <div style={{ paddingBottom: '0px', display: 'flex', alignItems: 'center' }}>{filterArray?.length > 0 && <img src={'/excel.png'} onClick={() => downloadExcelLedgerData()} style={{ height: '35px', width: '35px', objectFit: 'contain', cursor: 'pointer' }} alt='#    excel.png' title='Download Excel' />}</div>
//                         </div>
//                     }
//                 </div>
//             }
