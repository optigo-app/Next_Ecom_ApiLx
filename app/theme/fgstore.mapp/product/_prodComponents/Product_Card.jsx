...
import { Card, CardMedia, Checkbox, Skeleton } from "@mui/material";
import LocalMallOutlinedIcon from "@mui/icons-material/LocalMallOutlined";
import LocalMallIcon from "@mui/icons-material/LocalMall";
import StarBorderIcon from "@mui/icons-material/StarBorder";
import StarIcon from "@mui/icons-material/Star";

const Product_Card = ({ productData, isshowDots, menuParams, handleCartandWish, cartArr, wishArr, metalColorType, maxwidth590px, loginUserDetail, handleMoveToDetail, selectedMetalId, productIndex, yellowImage, whiteImage, roseImage, yellowRollImage, whiteRollImage, roseRollImage, imageUrl, videoUrl, RollImageUrl, location, metalColorCombo, storeInit }) => {
  ...
  return (
    <>
      {!isshowDots && productIndex === 6 && (
        <>
          {menuParams?.menuname === "Glossy" && (
            <div className="smr_productCard_banner">
              <img src={`${storImagePath()}/images/HomePage/ProductListing/5.png`} loading="lazy" alt="" draggable={true} onContextMenu={(e) => e.preventDefault()} />
            </div>
          )}
          {menuParams?.menuname === "Amber" && (
            <div className="smr_productCard_banner">
              <video src={`${storImagePath()}/images/HomePage/ProductListing/staticv1.mp4`} autoPlay muted controls={false} loop draggable={true} onContextMenu={(e) => e.preventDefault()}></video>
            </div>
          )}
          {menuParams?.menuname === "Ruby" && (
            <div className="smr_productCard_banner">
              <video src={`${storImagePath()}/images/HomePage/ProductListing/staticv2(1).mp4`} autoPlay muted controls={false} loop draggable={true} onContextMenu={(e) => e.preventDefault()}></video>
            </div>
          )}
        </>
      )}
      {!isshowDots && productIndex === 14 && (
        <>
          {menuParams?.menuname === "Glossy" && (
            <div className="smr_productCard_banner">
              <img src={`${storImagePath()}/images/HomePage/ProductListing/static4.jpg`} alt="Banner 2" loading="lazy" draggable={true} onContextMenu={(e) => e.preventDefault()} />
            </div>
          )}
          {menuParams?.menuname === "Amber" && (
            <div className="smr_productCard_banner">
              <video src={`${storImagePath()}/images/HomePage/ProductListing/staticv1(1).mp4`} autoPlay muted controls={false} loop draggable={true} onContextMenu={(e) => e.preventDefault()} />
            </div>
          )}
          {menuParams?.menuname === "Ruby" && (
            <div className="smr_productCard_banner">
              <video src={`${storImagePath()}/images/HomePage/ProductListing/staticv2.mp4`} autoPlay muted controls={false} loop draggable={true} onContextMenu={(e) => e.preventDefault()} />
            </div>
          )}
        </>
      )}
      <div className="smr_productCard">
        ...
        <div onMouseMove={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)} className="smr_ImgandVideoContainer" onClick={() => handleMoveToDetail(productData)}>
          {isLoading === true ? (
            <Card
              sx={{
                height: "100%",
              ...
              <div style={{ display: isHover ? "block" : "none" }}>
                {videoUrl !== undefined ? (
                  ...
                  <img
                    ...
                    src={selectedMetalColor === 1 ? yellowRollImage : selectedMetalColor === 2 ? whiteRollImage : selectedMetalColor === 3 ? roseRollImage : RollImageUrl}
                    onError={(e) => {
        ...
        <div className="smr_prod_card_info" style={{ height: !isshowDots ? "106px" : "90px" }}>
          <div className="smr_prodCard_1">
              ...
              <div className="smr_productList_metaltype_Maindiv">
                <div className="smr_productList_metaltype_div">
                  {metalColorType?.map((item) => (
                    <button className={selectedMetalColor === item?.id ? `smr_metaltype_${item?.metal}_clicked` : `smr_metaltype_${item?.metal}`} key={item?.id} type="button" disabled={yellowImage === undefined} onClick={() => handleClick(item?.id)}>
                      {""}
                    </button>
                  ))}
                </div>
            ...
            <div className="smr_prod_Title">
              <span className={productData?.TitleLine?.length > 30 ? "smr1_prod_title_with_width" : "smr1_prod_title_with_no_width"}>
                <strong style={{ color: "black" }}>{productData?.designno && productData?.designno !== "" ? productData?.designno : ""}</strong>
                {productData?.designno && productData?.designno !== "" && formatTitleLine(productData?.TitleLine) ? " - " : ""}
              ...
              >
                {storeInit?.IsGrossWeight == 1 && Number(productData?.Gwt) !== 0 && (
                  <span className="smr_prod_wt">
                    <span className="smr_main_keys">GWT:</span>
                    <span className="smr_main_val">{productData?.Gwt?.toFixed(3)}</span>
                  </span>
                )}
                {Number(productData?.Nwt) !== 0 && (
                  <>
                    ...
                    <span className="smr_prod_wt">
                      ...
                      <span className="smr_main_val">{productData?.Nwt?.toFixed(3)}</span>
                    </span>
                  </>
                )}
                {storeInit?.IsDiamondWeight == 1 && Number(productData?.Dwt) !== 0 && (
                  <>
                    <span style={{ fontSize: "13px" }}>|</span>
                    <span className="smr_prod_wt">
                      <span className="smr_main_keys">DWT:</span>
                ...
                {storeInit?.IsStoneWeight == 1 && Number(productData?.CSwt) !== 0 && (
                  <>
                    <span style={{ fontSize: "13px" }}>|</span>
                    <span className="smr_prod_wt">
                      <span className="smr_main_keys">CWT:</span>
                      <span className="smr_main_val">
            ...
            <div className="smr_prod_mtcolr_price">
              <span className="smr_prod_metal_col">
                {findMetalColor(productData?.MetalColorid)?.[0]?.metalcolorname.toUpperCase()}-{findMetalType(productData?.IsMrpBase == 1 ? productData?.MetalPurityid : (selectedMetalId ?? productData?.MetalPurityid))[0]?.metaltype}
              </span>
              {storeInit?.IsPriceShow == 1 && (
                <>
                  ...
                  <span className="smr_price">
                    ...
                    <span className="smr_pricePort">{formatter(productData?.UnitCostWithMarkUp)}</span>
                  </span>
                </>
              )}
            </div>
