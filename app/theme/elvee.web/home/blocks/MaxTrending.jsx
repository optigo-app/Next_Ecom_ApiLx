...
import { Box, Typography, Grid, Button, styled, Card, CardMedia } from "@mui/material";
...
const MaxTrending = ({ data, storeInit }) => {
    ...
    const fetchAndSetTrending = useCallback(async (finalID, precomputedKey) => {
        ...
        try {
            const localCacheRes = await fetch(`/api/v1/cache?mode=meta&key=${effectiveKey}`).then(res => res.json()).catch(() => ({ cached: false }));
            const matchingServerEntry = findMatchingCacheEntry(cacheList?.Data?.rd ?? [], pricingContext, eventName, apiALC);
            ...
            if (records.length > 0) {
                ...
                if (bookResult?.CacheRebuildDate) {
                    fetch("/api/v1/cache", {
                        ...
                        body: JSON.stringify({ key: effectiveKey, data: records, meta: { ...meta, CacheRebuildDate: bookResult.CacheRebuildDate } }),
                    });
                }
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        setValidatedData(trandingViewData.map(item => ({
            ...item,
    ...
    const handleNavigation = (item) => {
        let obj = {
            ...
            l: item?.ImageExtension, count: item?.ImageCount,
        };
                        ...
                        <BannerOverlay>
                            <Button
                                ...
                                sx={{
                                    ...
                                    "&:hover": { bgcolor: "#f8f8f8", boxShadow: "0 6px 20px rgba(0,0,0,0.15)" },
                                }}
                            >
                                SHOP COLLECTION
                            </Button>
                                ...
                                <ProductCardWrapper onClick={() => handleNavigation(item)}>
                                    <ImageBox>
                                        <Box
                                            ...
                                            component="img"
                                            src={item.ImageCount >= 1 ? item.validatedImageURL : imageNotFound}
