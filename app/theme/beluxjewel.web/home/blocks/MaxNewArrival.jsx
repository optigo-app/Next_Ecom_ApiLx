...
const MaxNewArrival = ({ storeInit, initialData = [] }) => {
  const prevRef = useRef(null);
  const nextRef = useRef(null);

  const [imageUrl, setImageUrl] = useState(storeInit?.CDNDesignImageFol || "");
  const [newArrivalData, setNewArrivalData] = useState(initialData || []);
  ...
  const validatedData = useMemo(() => {
    if (!newArrivalData || !newArrivalData.length) return [];
    const cdnFol = storeInit?.CDNDesignImageFol || imageUrl || "";
    return newArrivalData.map((item) => {
  ...
  }, [newArrivalData, getCardImageUrl, storeInit, imageUrl]);
  ...
  const fetchAndSetNewArrivals = useCallback(async () => {
    ...
    try {
      ...
      console.error("[MaxNewArrival] Error fetching new arrivals:", err);
      setLoading(false);
    } finally {
  ...
  useEffect(() => {
    ...
    if (lastUserRef.current !== currentUserSig || newArrivalData.length === 0) {
      lastUserRef.current = currentUserSig;
      fetchAndSetNewArrivals();
    }
  }, [islogin, loginUserDetail, storeInit, initialData, fetchAndSetNewArrivals, newArrivalData.length]);
