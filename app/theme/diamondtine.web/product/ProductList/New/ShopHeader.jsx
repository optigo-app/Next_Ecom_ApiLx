...
const ShopHeader = ({
  ...
  const decodedStr = Array.isArray(decodedSearchResult)
    ...
    : typeof decodedSearchResult === "string"
      ? decodedSearchResult.toLowerCase()
      : "";

  const hasCollection = decodedStr.includes("collection");
  const hasNewArrival = decodedStr.includes("newarrival");
...
{!isBelow768 && (
  <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
    ...
    >
      <Select
        value={sortingSelect ?? ""} // <-- Fix: Provide fallback empty string
        onChange={(e) => {
          handleSortby(e);
          handleChangeTrend(e);
    ...
    {storeinit?.IsMetalCustomization === 1 && (
      ...
      >
        <Select
          value={selectedMetalId ?? ""} // <-- Fix: Provide fallback empty string
          onChange={(e) => {
    ...
    {storeinit?.IsDiamondCustomization === 1 && (
      ...
      >
        <Select
          value={selectedDiaId ?? ""} // <-- Fix: Provide fallback empty string
          onChange={(e) => {
      ...
      >
        <Box
  component="div" // <-- Use component="div" or a plain Box to avoid generating a <p> tag
  sx={{
    display: "flex",
    alignItems: "center",
    gap: 1,
...
>
  <svg className="icon icon-filter" width="21" height="20" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
  ...
  >
    Filter
        ...
        {anyFilterApplied && (
          ...
          >
            <Box sx={{ width: "1px", height: "14px", bgcolor: "#ddd", mr: 1.5 }} />
