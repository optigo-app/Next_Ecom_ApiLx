...
const FilterAccordion = ({ title, items, checkedItems, handleCheckboxChange }) => {
  return (
    <Accordion className='hoq_cart-Accordion' style={{ boxShadow: 'none', border: 'none' }}>
      <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{ padding: '0' }}>
        <Typography className='hoq_FilterTitle'>{title}</Typography>
      </AccordionSummary>
      <AccordionDetails className='hoq_filterAccordianDetail' style={{ padding: '0' }}>
        {items?.map(item => (
          <div className='hoq_filter-AccrodianDetails' key={item.id}>
            <Typography className='hoq_filterAccordianDeatilTitle'>{item.name}</Typography>
            <Checkbox
              checked={checkedItems[title]?.[item.id] || false}
              onChange={() => handleCheckboxChange(item.id, title)}
              className='hoq_filterCheckbox'
            />
          </div>
        ))}
      </AccordionDetails>
...
const MyModal = ({ open, onClose, selectedItems, onRemove, onUpdateCart, onCancelCart }) => {
  ...
  return (
    ...
    >
      <div className="hoq_paper">
        <div className='hoq_Modal_Title'>
          <Typography className='hoq_Modal_TitleTypo' variant="h5" id="modal-title" gutterBottom>
            Update Cart
          </Typography>
          <IconButton className="hoq_closeIcon" onClick={onClose}>
            <CloseIcon />
          </IconButton>
        </div>
        <Divider />
        <div className='hoq_cartmodal_body'>
          <div>
            <div className="hoq_ShortCutCombo-section">
              <div>
          ...
          <div className='hoq_cartModal-Divider'>
            <Divider />
          </div>
          <Grid container spacing={2}>
            <Grid item xs={6} md={3}>
              <div className='hoq_Modal-FilterList'>
                {generateFilterAccordions(filterArr, checkedItems, handleCheckboxChange)}
              </div>
            </Grid>
            <Grid item xs={6} md={9}>
              <div className='hoq_Modal-cardList'>
                <Grid container spacing={2}>
                  {filteredItems?.map(product => (
                    <Grid item key={product.id} xs={12} sm={4} md={4}>
                      <Card className='hoq_cartListCard' sx={{ maxWidth: 250, position: 'relative' }}>
                        <CardActionArea>
                          <CardMedia
                            component="img"
                          ...
                          <CardContent>
                            ...
                            <div className='designNocartList'>
                              <p className='hoq_DesignNoTExt'>{product?.designno}</p>
                            </div>
          ...
          <CardActions className='hoq_projectUpdateCartBtn-group'>
            <div className="hoq_projectUpdateCartBtn-group">
              <button className="hoq_cartUpdate-btn" onClick={() => onUpdateCart(filteredItems)}>
                Update
              </button>
              <button className="hoq_cartCancel-btn" onClick={onCancelCart}>
                Cancel
              </button>
            </div>
          </CardActions>
