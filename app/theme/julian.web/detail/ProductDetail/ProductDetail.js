...
const ProductDetail = ({ storeinit, searchParams, params }) => {
    ...
    if (initialDecodeUrl && Object.keys(initialDecodeUrl).length > 0) {
      try {
        ...
        if (resolvedImages.length > 0) {
          ...
          setThumbImgIndex(0);
          // Do NOT call setMediaBuildDone(true) here — wait for ProdCardImageFunc
          // which uses authoritative singleProd.MetalColorid from API
        }
  ...
  useEffect(() => {
    try {
      if (selectedThumbImg && selectedThumbImg?.link?.imageUrl) {
        setImageSrc(selectedThumbImg?.link?.imageUrl);
      } else if (initialDecodeUrl?.img && !initialDecodeUrl.img.includes("undefined")) {
        setImageSrc(initialDecodeUrl.img);
      }
    } catch (error) {
      console.log("Error in fetching image", error);
    }
  }, [selectedThumbImg, initialDecodeUrl]);
  ...
  if (resolvedImages.length === 0 && dNo && baseFol) {
    const hasColorInImg = Boolean(
      validPassedImg &&
      activeColorCode &&
      validPassedImg.toLowerCase().includes(`~${activeColorCode.toLowerCase()}.`)
    );
    const colorSuffix = (activeColorCode && hasColorInImg) ? `~${activeColorCode}` : "";
